...
### Per-project one-pager (`magna.html` / `stratos.html` / `ivory.html` / `wahed.html` / `ipg.html` / `passion-projects.html`)
...
2. **Hero** — split layout: left = company name in Italiana + role + tenure + category, set on the company brand color band; right = paper-color synopsis + **"Download case study" CTA** linking to `case-studies/{slug}.pdf`.
...
6. **Campaigns** — sequence of `.campaign-block` components per `§4 CP-1`, one per asset folder under `assets/campaigns/{slug}/`. Until real assets exist, render a single "drop assets here" placeholder card.
...
### Campaigns & assets — invariants
- **CP-1 — Campaign block component.** Every campaign on a per-project page uses this exact shape: asset on one side (reel video *or* Instagram carousel embed), metadata on the other (small-caps label, Italiana title, single-sentence purpose, 2–4 outcome numbers). No variations, no bespoke per-campaign layouts. Stacks to single column on mobile.
  - Reel: local `<video>` element with `muted playsinline loop` + `data-autoplay-on-scroll` attribute. Behavior: paused until ≥50% in viewport, autoplays muted on entry, pauses on exit (IntersectionObserver).
  - Carousel: Instagram embed via `<blockquote class="instagram-media">` + `//www.instagram.com/embed.js` script.
- **AS-1 — Asset directory convention.** All campaign assets live under `assets/campaigns/{company-slug}/{NN-campaign-slug}/` where `NN` is a two-digit ordering prefix. Each folder contains either `reel.mp4` + `poster.jpg` (reel) or *no media files* (carousel — IG link lives in `meta.md`), plus a required `meta.md` describing the campaign. Customer never writes HTML; customer drops files into folders. Implementer reads `meta.md` and writes the campaign block in the corresponding per-project page.
- **PDF-1 — Per-project PDF export.** Each per-project page (`magna.html`, `stratos.html`, `ivory.html`, `wahed.html`, `ipg.html`, `passion-projects.html`) has a generated PDF artefact at `case-studies/{slug}.pdf`. PDFs are generated by `pdf.js` (Playwright print-to-PDF of the per-project URL) with `@media print` styles applied. Hand-editing a PDF is forbidden — the HTML is the single source of truth. The page hero carries a "Download case study" CTA linking to the PDF.
...
mehr-portfolio/
├── index.html             ← cover
├── approach.html          ← thesis page
├── styles.css             ← global system, all pages
├── magna.html             ← per-project pages, each links styles.css
├── stratos.html
├── ivory.html
├── wahed.html
├── ipg.html
├── passion-projects.html
├── pdf.js                 ← Playwright print-to-PDF (per SPEC §4 PDF-1)
├── assets/
│   └── campaigns/
│       ├── magna/
│       │   ├── 01-{slug}/
│       │   │   ├── reel.mp4    (or omitted if carousel)
│       │   │   ├── poster.jpg  (reel cover)
│       │   │   └── meta.md     (required)
│       │   └── 02-{slug}/...
│       ├── stratos/...
│       ├── ivory/...
...
│       └── passion-projects/...
└── case-studies/
    ├── magna.pdf          ← generated; never hand-edit
    ├── stratos.pdf
    ├── ivory.pdf
    ├── wahed.pdf
    ├── ipg.pdf
    └── passion-projects.pdf
```
